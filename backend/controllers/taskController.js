...
const updateTaskChecklist = async (req,res) => {
    try{
        ...
        if (!task.assignedTo.includes(req.user._id) && req.user.role !=="admin"){
            return res
            .status(403)
            .json({message:"You are not Authorized to Update the checklist"});
        }

        task.todoChecklist = todoChecklist; //Replace it with updated checklist
...
const getDashBoardData = async (req,res) => {
    try{
        ...
        const taskPriorityLevels = taskPriorities.reduce((acc,priority)=>{
            acc[priority] = 
                taskPriorityLevelsRaw.find((items)=>items._id===priority)?.count ||0;
            return acc;    
        },{});
