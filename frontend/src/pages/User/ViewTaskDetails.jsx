...
const ViewTaskDetails = () => {
  ...
  // Update TODO checklist (SAFE)
  const updateTodoChecklist = async (index) => {
    if (!task || loadingTodo) return;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTask({ ...task, todoChecklist: previousChecklist });
      }
    } catch (error) {
            ...
            <div className='form-card col-span-3'>
              ...
              <div className='grid grid-cols-12 gap-4 mt-4'>
                ...
                <div className='col-span-6 md:col-span-4'>
                  ...
                  <AvatarGroup
                    avatars={task.assignedTo?.map(u => u?.profileImage) || []}
                    maxVisible={5}
                  />
                </div>
              </div>
...
const TodoChecklist = ({ text, isChecked, onChange, disabled }) => (
  <div className='flex items-center gap-3 mt-2'>
    <input
      ...
      onChange={onChange}
      disabled={disabled}
      className='w-4 h-4 text-indigo-500 bg-gray-100 border border-gray-300 rounded-sm cursor-pointer'
    />
